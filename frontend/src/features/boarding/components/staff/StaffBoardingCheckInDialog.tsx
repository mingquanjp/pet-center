...
export function StaffBoardingCheckInDialog({
  isOpen,
  isPending = false,
  onOpenChange,
  onSubmit,
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-105 bg-white rounded-3xl p-0 overflow-hidden border-0! shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] outline-none">
        <DialogHeader className="px-7 pt-7 pb-5">
          <DialogTitle className="text-xl font-bold tracking-tight text-[#111827]">Tiến hành Check-in</DialogTitle>
          <DialogDescription className="text-[15px] leading-relaxed text-[#4B5563] mt-2">
            Xác nhận tiếp nhận thú cưng vào trung tâm? Phiếu lưu trú sẽ chuyển sang trạng thái{" "}
            <span className="font-medium text-[#111827]">Đang lưu trú</span>.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter className="px-7 py-5 bg-[#F9FAFB] flex sm:justify-end gap-3 sm:gap-3 border-t border-[#F3F4F6]">
          ...
          >
            {isPending ? "Đang xử lý..." : "Check-in ngay"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
